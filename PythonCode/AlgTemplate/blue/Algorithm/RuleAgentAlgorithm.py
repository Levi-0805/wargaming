...
class MultiObjectivePlan:
    """按Python的team_index动作行号，把守军稳定分配给多个目标。"""
...
class ReinforceAgentAlgorithm(RuleAlgorithm):
    ...
    def defense_point(self, state: TeamState, agent):
        """按实体编号和兵种生成多个指挥所周围的防御点。"""
        ...
        radius = 700.0 + 300.0 * (slot % 3)
        height = 1800.0 if agent.entity_type == "BP_Base_UAV_C" else 0.0
        return (
            float(objective.position[0]) + radius * math.cos(angle),
            float(objective.position[1]) + radius * math.sin(angle),
    ...
    def choose_action(self, state: TeamState, agent) -> Action | ChangeParentAction:
        ...
        return Action.guard_at(self.defense_point(state, agent))
