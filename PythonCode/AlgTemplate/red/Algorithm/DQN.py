...
class MultiObjectivePlan:
    """用Python的team_index动作行号稳定分配目标，避免奖励跳变。"""
...
class DQNActionSet(ActionSetDefinition):
    """红方精简动作集合：移动、局部攻击和切换上级。"""

    defending_team = 1

    def __init__(self):
        # 攻击目标在当前帧动态解析，不再为每个敌人扩展动作槽。
        super().__init__("red_minimal_control", version=1, include_target_attacks=False)
...
class DQNAgentAlgorithm(DQNAlgorithm):
    """完整红方 DQN 案例：向蓝方指挥所推进、占领并打击可见敌方。"""

    defending_team = 1
    capture_radius = 1500.0
    ...
    def calculate_reward(
        ...
        for index, (old_agent, new_agent) in enumerate(
            ...
            if old_distance >= self.capture_radius > new_distance:
                reward[index] += 0.20
            if new_distance < self.capture_radius:
                reward[index] += 0.005
